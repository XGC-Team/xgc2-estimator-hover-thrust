...
enum class HoverThrustHealthCondition : uint8_t {
    kInputUnhealthy = 0,
    kBelowMinAltitude = 1,
    kEstimationReady = 2,
};
...
struct HoverThrustHealthStatus {
    HoverThrustHealthCondition condition{HoverThrustHealthCondition::kInputUnhealthy};
    uint32_t flags{0};
    bool ready{false};
    double source_stamp_sec{0.0};
};

enum class HoverThrustDebugTracePhase : uint8_t {
    kTransitionPass = 1,
    kTickPass = 2,
};

struct HoverThrustDebugTraceRecord {
    HoverThrustDebugTracePhase phase{HoverThrustDebugTracePhase::kTransitionPass};
    ::state_machine::EventTraceRecord trace;
};
